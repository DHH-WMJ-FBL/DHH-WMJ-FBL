...
private:
    ...
    int countPiecesBetween(int targetX, int targetY, ChessMan* board[10][9]) const {
        ...
        if (maxDistance > 20) { // 设置一个合理的最大距离
            qDebug() << "炮移动距离异常大:" << maxDistance << "，起点:(" << m_x << "," << m_y << ")，终点:(" << targetX << "," << targetY << ")";
            return 0;
        }
        ...
        if (targetX == m_x) {
            ...
            for (int y = m_y + step; y != targetY && safetyCounter < maxIterations; y += step, safetyCounter++) {
                // 添加边界检查
                if (y < 0 || y >= 10) {
            ...
            for (int x = m_x + step; x != targetX && safetyCounter < maxIterations; x += step, safetyCounter++) {
                // 添加边界检查
                if (x < 0 || x >= 9) {
